...
namespace NativeDeviceControlLib
{
    ...
    /// </summary>
    public class NativeDeviceContext
    {
        // Context data.
        ...
        public HCTX Open(HWND hwnd_I, bool enable_I)
        {
            try
            {      
                m_hCTX = NativeDeviceFunctions.WTOpenA(hwnd_I, ref m_logContext, enable_I);
            }
            catch (Exception ex)
            {
                MessageBox.Show("FAILED OpenContext: " + ex.ToString(), "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        public bool Open()
        {
            // Get the handle of the anonymous NativeDeviceMessageEvents window. This is a 
            // static (global) object, so there's only one of these at a time.
            HWND hwnd = NativeDeviceMessageEvents.WindowHandle;

            try
            {
                m_hCTX = NativeDeviceFunctions.WTOpenA(hwnd, ref m_logContext, true);
            }
            catch (Exception ex)
            {
                MessageBox.Show("FAILED OpenContext: " + ex.ToString(), "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public bool Close()
        {
            ...
            try
            {
                ...
                status = NativeDeviceFunctions.WTClose(m_hCTX);
                m_hCTX = 0;
                m_logContext = new WintabLogContext();
            }
            catch (Exception ex)
            {
                MessageBox.Show("FAILED CloseContext: " + ex.ToString(), "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        public bool Enable(bool enable_I)
        {
            ...
            try
            {
                ...
                status = NativeDeviceFunctions.WTEnable(m_hCTX, enable_I);
            }
            catch (Exception ex)
            {
                MessageBox.Show("FAILED EnableContext: " + ex.ToString(), "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        public bool SetOverlapOrder(bool toTop_I)
        {
            ...
            try
            {
                ...
                status = NativeDeviceFunctions.WTOverlap(m_hCTX, toTop_I);
            }
            catch (Exception ex)
            {
                MessageBox.Show("FAILED SetContextOverlapOrder: " + ex.ToString(), "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
